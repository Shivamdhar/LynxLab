...
This guide walks you through deploying the LynxLab stack using Terraform. The stack provisions a fully serverless AI inference API on AWS, protected by a custom Lambda authorizer.

For the alternative CloudFormation-based deployment, see the [root README](../README.md).
...
> Alternatively, create `~/.aws/config` manually — see the [root README](../README.md) for details.
...
**Cause:** The token in Secrets Manager does not match the value in your `Authorization` header. This can happen if the secret was previously set with a different value (e.g. from a prior `driver.py` run or a previous `terraform apply`).
...
- [Root README](../README.md) — CloudFormation deployment via `driver.py`
